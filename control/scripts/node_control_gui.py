...
try:
    ...
    # Assuming 'drive_control.src.steering' is available in ROS2
...
class ArmControlGUI(QWidget):
    ...
    def publish_drive_twist(self):
        print(
            f"Publishing twist - Linear: {self.rover_linear_vel}, Angular: {self.rover_angular_vel}"
        )
    ...
    def onChangeCmd(self, joint):
        ...
        if joint in self.joints["brushless"]:
            ...
            cmdMsgDrive.left = [0.0, 0.0]
            cmdMsgDrive.right = [0.0, 0.0]
    ...
    def ros_setup(self):
        # Publishers
        self.arm_brushless_cmd_publisher = self.node.create_publisher(
            Float32MultiArray, "/armBrushlessCmd", 10
        )
        self.arm_brushed_cmd_publisher = self.node.create_publisher(
            Float32MultiArray, "/armBrushedCmd", 10
        )
        self.drive_cmd_publisher = self.node.create_publisher(
            WheelSpeed, "/wheel_velocity_cmd", 10
        )
        self.drive_twist_publisher = self.node.create_publisher(
            Twist, "rover_velocity_controller/cmd_vel", 10
        )

        # Subscribers
        self.node.create_subscription(
            WheelSpeed, "/wheel_velocity_cmd", self.on_update_drive_cmd, 10
        )
        self.node.create_subscription(
            Float32MultiArray,
            "/armBrushlessFb",
            self.on_update_brushless_outshaft_fb,
            10,
        )
        self.node.create_subscription(
            Float32MultiArray,
        ...
        self.node.create_subscription(
            Float32MultiArray, "/armBrushedFb", self.on_update_brushed_fb, 10
        )
        self.node.create_subscription(
            WheelSpeed, "/wheel_velocity_feedback", self.update_drive_fb, 10
        )
...
def main():
    ...
    node = rclpy.create_node("arm_control_gui")
    gui = ArmControlGUI(node)
    gui.show()

    # Create a QTimer to spin the node
    timer = QTimer()
    timer.timeout.connect(lambda: rclpy.spin_once(node, timeout_sec=0))
    timer.start(10)  # Spin every 10 ms

    sys.exit(app.exec_())
    # Shutdown rclpy when the app exits
    rclpy.shutdown()
