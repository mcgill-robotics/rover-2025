import time
import pygame
from arm.archive.armCANCommunicationV2 import ArmNodeID, CANStation, ArmESCInterface
import json
    ...
    while True:
        ...
        if motor_type == "brushless":
            ...
            if joystick.get_button(TRIANGLE_BUTTON):
                ...
                try:
                    arm.calibrate_joint(ArmNodeID.WAIST)
                    time.sleep(0.025)
                    arm.calibrate_joint(ArmNodeID.SHOULDER)
                    time.sleep(0.025)
                    arm.calibrate_joint(ArmNodeID.ELBOW)
                    time.sleep(0.025)
                except Exception as e:
